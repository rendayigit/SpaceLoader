#include "CommonClientServer.h"

#include "../version.h"

CommonClientServer::CommonClientServer(QString yamlFile) {
    cmdsYamlFile = yamlFile;
    Log().setFlushRate(0);
}

bool CommonClientServer::parseMessage(QTcpSocket *sender, QByteArray message) {
    for (auto &i : cmdList) {
...
bool CommonClientServer::checkTimerFlag(vector<YAML::Node> cmdsWithTheTimerFlagSetNodeList,
...
bool CommonClientServer::checkAuthFlag(vector<YAML::Node> cmdsWithTheAuthFlagSetNodeList,
                                       QString cmdCallString) {
...
void CommonClientServer::populateCmdLists() {
    cmdList.clear();
...
void CommonClientServer::runBatchScript(QTcpSocket *sender, CallCmd *cmd, QString message) {
...
void CommonClientServer::timerTrigger() {
...
QString CommonClientServer::help() {
    QString helpString = "\n";
...
QString CommonClientServer::spaceloaderVersion() {

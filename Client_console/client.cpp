...
Client::Client() : Operations(Path::client_cmds_Yaml) {}
...
void Client::listenTo(QString msg) {
    ...
    vector<YAML::Node> nodeList = Yaml::getNodeByTag<vector<YAML::Node>>(
    ...
    if (nodeList.size() < 1) {
        ...
        ip = QString::fromStdString(Yaml::getText<vector<string>>(nodeList.at(0), "ip")[0]);
        port = QString::fromStdString(Yaml::getText<vector<string>>(nodeList.at(0), "port")[0]);
    }
...
void Client::stopListen(QString ipPort) {
    ...
    if (nodeList.size() < 1) {
        ...
        ipPort = QString::fromStdString(Yaml::getText<vector<string>>(nodeList.at(0), "ip")[0]) +
                 ":" +
                 QString::fromStdString(Yaml::getText<vector<string>>(nodeList.at(0), "port")[0]);
    }
...
void Client::help(QByteArray message) {
    if (cmp("help", message)) {
        ...
        QString helpString = "\t" + QString::fromStdString(Yaml::getText<vector<string>>(
                                        nodeList.at(0), "Description")[0]);
        helpString.insert(25, "- " + QString::fromStdString(
                                         Yaml::getText<vector<string>>(nodeList.at(0), "type")[0]));
        helpString = helpFormatter(helpString);
        std::cout << helpString.toStdString() << std::endl;
    }
}

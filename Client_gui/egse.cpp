...
Egse::Egse(Backend* backend) : backend(backend){};

void Egse::onReceived(QByteArray message) {
    backend->egseReplier(message.mid(6, message.size()));
...
void Egse::buttonCallback(QString buttonId) {
    ...
    if (buttonId.contains("STARKIT")) {
        interpret(
            "77 0C C0 00 00 0E 54 52 41 4E 53 46 45 52 20 52 45 4D 4F 54 45");  // TRANSFER REMOTE
        ...
        interpret(
            "77 0C C0 00 00 15 49 70 75 43 6D 64 20 74 63 67 2E 70 6C 6F 70 5F 6D 6F 64 65 20 32");  // IpuCmd tcg.plop_mode 2
        interpret("77 0C C0 00 00 0A 53 65 74 54 4D 6D 6F 64 65 20 31");  // SetTMmode 1

        interpret(
            "77 0C C0 00 00 0E 49 70 75 53 65 74 54 63 67 44 65 73 74 20 34");  // IpuSetTcgDest 4
        interpret(
            "77 0C C0 00 00 10 49 70 75 53 65 74 54 6D 61 53 6F 75 72 63 65 20 33");  // IpuSetTmaSource
                                                                                      // 3
        ...
        interpret(
            "77 0C C0 00 00 0E 49 70 75 53 65 74 54 63 67 44 65 73 74 20 32");  // IpuSetTcgDest 2
        interpret(
            "77 0C C0 00 00 10 49 70 75 53 65 74 54 6D 61 53 6F 75 72 63 65 20 32");  // IpuSetTmaSource
                                                                                      // 2

                ...
                if (event.key === Qt.Key_Return || event.key === Qt.Key_Enter) {
                    ...
                    for (var i = 0; i < textList.length; i++) {
                        ...
                        if(textList[i].lastIndexOf(keyword) == textList[i].length - keyword.length - 1) {
                            edittedText += "<font color='#0000ff'>" + keyword + "</font>"
                            cursorList.push(count)
                            count += keyword.length
                        }

                        //if(searchList.length == 0){
                         //   edittedText = textList
                        //}//Not complete...supposed to return original log when search bar is empty without the 'undefined'
